...
class _QuotationScreenState extends State<QuotationScreen> with BarcodeScanMixin {
  ...
  Widget _gradientBorder({required Widget child, double radius = 10, Color fill = Colors.white}) {
    return Container(
      decoration: BoxDecoration(gradient: _brandGradient, borderRadius: BorderRadius.circular(radius)),
      ...
      actions: [
        PopupMenuButton<int>(
          ...
          child: Container(
            ...
            child: Text('$_power',
                style: GoogleFonts.poppins(color: Colors.red, fontSize: 14, fontWeight: FontWeight.bold)),
          ),
        ),
  ...
  Widget _buildCustomerInput(QuotationViewModel vm) {
    final s = context.s;
            ...
            Container(
              ...
              child: visible.isEmpty
                  ? Padding(
                      ...
                      child: Text(s.noItemsFound, style: GoogleFonts.poppins(color: Colors.grey, fontSize: 13)),
                    )
                  : ListView.separated(
                      shrinkWrap: true,
  ...
  Widget _buildItemCodeRow(QuotationViewModel vm) {
    final s = context.s;
  ...
  Widget _buildProductTable(QuotationViewModel vm) {
    ...
    return Container(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          // Fill tablet width so Action sits flush after RFID (no white gap).
          // On narrow phones keep min width + horizontal scroll.
          final availableForData = (constraints.maxWidth - _actionWidth).clamp(0.0, double.infinity);
          final minDataWidth = _cellWidth * _dataColCount;
          final dataWidth = availableForData > minDataWidth ? availableForData : minDataWidth;
          final cellW = dataWidth / _dataColCount;

          Widget cell(String text, {Color color = Colors.black54, FontWeight weight = FontWeight.normal}) =>
              _tableCell(text, color: color, weight: weight, width: cellW);
                    ...
                    child: Column(
                      ...
                      children: [
                        Container(
                          ...
                          child: Row(
                            children: headers
                                .map((h) => cell(h, color: Colors.white, weight: FontWeight.w600))
                                .toList(),
                          ),
                        ),
                        Expanded(
                                      ...
                                      child: Container(
                                        ...
                                        child: Row(
                                          children: [
                                            ...
                                            cell(item.grWt ?? ''),
                                            cell(item.nWt ?? ''),
                                            cell(item.finePlusWt ?? ''),
                                            cell(item.stoneAmt ?? ''),
                                            cell(item.diamondAmt),
                                            cell(item.itemAmt ?? ''),
                                            cell(item.rfidCode),
                                          ],
                                        ),
                        ...
                        Container(
                          ...
                          child: Row(
                            children: totals
                                .map((t) => cell(t, color: Colors.white, weight: FontWeight.bold))
                                .toList(),
                          ),
                        ),
                      ],
  ...
  Widget _buildSummaryRow(QuotationViewModel vm) {
    final s = context.s;
    return Container(
      ...
      child: Scaffold(
        ...
        bottomNavigationBar: ScanBottomBar(
          ...
          onSave: () async {
            final s = context.sRead;
            try {
              final res = await vm.submitQuotation();
              if (!mounted) return;
              if (res != null) {
          ...
          onList: () => Navigator.pushNamed(context, '/quotation_list'),
          onScan: () async {
